...
from app.telegram.messages import near_sl_message, profit_message, signal_message, tp_message
...
class TradingService:
    ...
    Scheduler calls monitor_once() and scheduled reports; it never creates signals.
    """
    ...
    async def refresh(self):
        ...
        self.scan_cursor = min(self.scan_cursor, max(0, len(self.universe) - 1))
    ...
    def _rows_to_df(payload):
        ...
        for column in df.columns:
            ...
            if key in ("o", "open"): rename_map[column] = "open"
            elif key in ("h", "high"): rename_map[column] = "high"
            elif key in ("l", "low"): rename_map[column] = "low"
            elif key in ("c", "close"): rename_map[column] = "close"
            elif key in ("v", "volume"): rename_map[column] = "volume"
        df = df.rename(columns=rename_map)
        required = {"open", "high", "low", "close", "volume"}
        return df if required.issubset(df.columns) else None

    async def scan_once(self, source="telegram"):
        """Manual discovery only. Never called by Scheduler."""
        ...
        async with self.scan_lock:
            ...
            if not self.s.paper_mode:
                return "🛑 PAPER_MODE غير مفعّل؛ تم منع إنشاء الصفقة حفاظًا على السلامة."
            if not self.can_send():
                return "ℹ️ لا توجد إشارة جديدة: تم بلوغ حد الصفقات أو الإشارات اليومية."

            if not self.universe or not self.last_refresh or (self._utc_now() - self.last_refresh).total_seconds() > self.s.universe_refresh_seconds:
                try:
                    await self.refresh()
                except Exception as exc:
                    print(f"[universe] refresh failed: {exc}")
                    return "⚠️ تعذر تحديث قائمة أسهم TASI حاليًا."

            market_data = await self._market()
            if market_data is None:
                return "⚠️ بيانات السوق غير متاحة حاليًا."
            regime = classify_tasi(market_data)

            batch = self._next_batch(self.s.manual_quotes_per_signal, "scan_cursor")
            print(f"[manual-scan] source={source} quotes={len(batch)} cursor={self.scan_cursor}/{len(self.universe)}")
            candidates = []
            ...
            for item in batch:
                ...
                try:
                    ...
                    if quote.updated_at:
                        age = (self._utc_now() - quote.updated_at).total_seconds() / 60
                        if age > self.s.data_max_delay_minutes:
                            print(f"[quote] {symbol} skipped: stale {age:.1f}m")
                            continue
                    candidate = fast_score(quote, regime)
                    if candidate.score >= max(55, self.s.min_score - 10):
                        candidates.append((candidate, item))
                except Exception as exc:
                    print(f"[quote] {symbol} failed: {exc}")

            if not candidates:
                return (
                    f"🔎 انتهى الفحص اليدوي. تم فحص {len(batch)} سهمًا من أصل {len(self.universe)}.\n"
                    "لم تظهر فرصة أولية تستحق التحليل العميق."
                )

            candidates.sort(key=lambda x: x[0].score, reverse=True)
            engine = SignalEngine(self.s, self.store.history())

            for candidate, item in candidates[: min(3, len(candidates))]:
                symbol = candidate.quote.symbol
                try:
                    historical = await self.p.historical(symbol, 250)
                    df = self._rows_to_df(historical)
                    if df is None:
                        print(f"[history] {symbol}: insufficient OHLCV")
                        continue
                    features = latest_features(df)
                    signal = engine.build(candidate, regime, item.get("sector", ""), features)
                    if not signal:
                        continue
                    if self.trade_manager.add(signal):
                        ...
                        return (
                            f"✅ تم اكتشاف وإرسال إشارة جديدة: {signal.name} ({signal.symbol})\n"
                            f"📊 Score: {signal.score:.1f} | Probability: {signal.probability:.1f}%\n"
                            f"📡 SAHMK delayed | تم فحص {len(batch)} سهمًا في هذه الجولة."
                        )
                except Exception as exc:
                    print(f"[analysis] {symbol} failed: {exc}")

            return (
                f"🔎 اكتمل الفحص اليدوي. تم فحص {len(batch)} سهمًا من أصل {len(self.universe)}.\n"
                "لم توجد صفقة مستوفية لجميع الشروط."
            )
    ...
    async def monitor_once(self):
        ...
        async with self.monitor_lock:
            ...
            batch_size = max(1, int(self.s.trade_monitor_quotes_per_cycle))
            trades = state["open_trades"]
            selected = []
            total = len(trades)
            start = self.monitor_cursor % total
            for i in range(min(batch_size, total)):
                selected.append(trades[(start + i) % total])
            ...
            for trade in selected:
                ...
                try:
                    ...
                    # Profit thresholds are sent once per trade.
                    pct = (quote.price - float(updated["entry"])) / float(updated["entry"]) * 100
                    sent = set(updated.get("profit_alerts_sent", []))
                    try:
                        thresholds = [float(x.strip()) for x in self.s.profit_alert_thresholds.split(",") if x.strip()]
                    except ValueError:
                        thresholds = [2, 5, 10, 15, 20]
                    for threshold in thresholds:
                        if pct >= threshold and threshold not in sent:
                            await self.b.send_profit(profit_message(updated, quote.price, quote.price - float(updated["entry"])))
                            sent.add(threshold)

                    # Near-SL warning is emitted once before SL.
                    stop = float(updated.get("trailing_stop") or updated["sl"])
                    distance_pct = abs(quote.price - stop) / float(updated["entry"]) * 100
                    if quote.price > stop and distance_pct <= self.s.near_sl_warning_pct and not updated.get("near_sl_warning_sent"):
    ...
    async def scheduled_tasks(self):
        """Periodic tasks that are safe under the free-data quota."""
    ...
    async def _scheduled_market_close_message(self):
        ...
        if key == self.last_market_close_key or not (local.hour == int(self.s.market_close.split(":")[0]) and local.minute >= int(self.s.market_close.split(":")[1])):
            return
    ...
    async def _scheduled_weekly_report(self):
        ...
        if not self.s.weekly_report_enabled:
            return
        if local.weekday() != self.s.weekly_report_weekday:
            return
    ...
    def open_trades_text(self):
        ...
        for t in trades:
            lines.append(
                ...
                f"SL: {float(t['sl']):.2f} | TP1: {float(t['tp1']):.2f} | TP2: {float(t['tp2']):.2f} | TP3: {float(t['tp3']):.2f}"
            )
        return "\n\n".join(lines)
    ...
    async def health_text(self):
        ...
        try:
            sahmk_ok = (await self._market()) is not None
        except Exception as exc:
            print(f"[health] SAHMK failed: {exc}")
        stats = self.p.stats() if hasattr(self.p, "stats") else {}
        return (
            "🟢 SYSTEM HEALTH\n\n"
            f"Telegram: {'OK' if telegram_ok else 'ERROR'}\n"
            f"SAHMK: {'OK' if sahmk_ok else 'ERROR'}\n"
            "Scheduler: RUNNING (monitor only)\n"
            f"Paper Mode: {'ON' if self.s.paper_mode else 'OFF'}\n"
            f"Universe: {len(self.universe)}\n"
            f"Open Trades: {len(state['open_trades'])}\n"
            f"Last Manual Scan: {state['meta'].get('last_scan', '—')}\n"
            f"Last Universe Update: {state['meta'].get('last_universe_refresh', '—')}\n"
            f"SAHMK Requests: {stats.get('requests', '—')} | 429: {stats.get('rate_limits', '—')}"
        )
    ...
    def settings_text(self):
        return (
            ...
            f"Min Probability: {self.s.min_probability}%\n"
            f"Max Daily Signals: {self.s.max_daily_signals}\n"
            f"Max Open Trades: {self.s.max_open_trades}\n"
            f"Manual Quote Cap: {self.s.manual_quotes_per_signal}\n"
            f"Monitor Quote Cap/Cycle: {self.s.trade_monitor_quotes_per_cycle}\n"
