...
class Settings(BaseSettings):
    ...
    telegram_mode: str = "auto"  # auto -> webhook on Render, polling locally

    sahmk_api_key: str
    sahmk_base_url: str = "https://api.sahmk.sa/api/v1"
    sahmk_plan: str = "free"
    sahmk_min_request_interval: float = 6.5
    sahmk_local_daily_limit: int = 95

    state_dir: str = "data"
    health_interval: int = 600

    # Scheduler is monitoring/reporting only. It NEVER discovers new signals.
    scan_interval_seconds: int = 900
    trade_monitor_quotes_per_cycle: int = 1

    # /signal screens up to 50 active stocks from the free market-volume endpoint,
    # then requests detailed single-stock quotes only for the strongest few.
